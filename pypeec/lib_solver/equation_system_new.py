...
def _get_system_size(A_net_c, A_net_m, A_src_c, A_src_m):
    """
    ...
    # get the source size
    (A_vc_src, A_src_vc, A_src_src) = A_src_c
    (n_src, n_src_c) = A_src_src.shape

    (A_vc_src, A_src_vc, A_src_src) = A_src_m
    (n_src, n_src_m) = A_src_src.shape

    # get the system size
    n_dof = n_vc+n_fc+n_src_c+n_vm+n_fm+n_src_m

    return n_vc, n_fc, n_src_c, n_vm, n_fm, n_src_m, n_dof


def _get_system_scaler(freq, n_fc, n_fm):
    """
    ...
    if freq == 0:
        scaler_fc = np.ones(n_fc, dtype=np.complex_)
        scaler_fm = np.ones(n_fm, dtype=np.complex_)
    else:
        scaler_fc = np.ones(n_fc, dtype=np.complex_)
        scaler_fm = s*np.ones(n_fm, dtype=np.complex_)

    return scaler_fc, scaler_fm


def _get_split_vector(sol, n_vc, n_fc, n_src_c, n_vm, n_fm, n_src_m):
    """
    Split a vector into an electric vector and magnetic vector.

    The input vector has the following size: n_fc+n_vc+n_src_c+n_src_v+n_fm+n_vm+n_src_m.
    The electric field vector has the following size: n_fc+n_vc+n_src_c.
    The electric source vector has the following size: n_fc+n_vc+n_src_c+n_src_v.
    The magnetic vector has the following size: n_fm+n_vm.
    """

    sol_c = sol[0:n_fc+n_vc]
    sol_src_c = sol[n_fc+n_vc:n_fc+n_vc+n_src_c]
    sol_m = sol[n_fc+n_vc+n_src_c:n_fc+n_vc+n_src_c+n_fm+n_vm]
    sol_src_m = sol[n_fc+n_vc+n_src_c+n_fm+n_vm:n_fc+n_vc+n_src_c+n_fm+n_vm+n_src_m]

    return sol_c, sol_src_c, sol_m, sol_src_m


def _get_split_field(sol, n_v, n_f):
    """
    Split an electric/magnetic vector into a face vector and voxel vector.

    The input vector has the following size: n_f+n_v.
...
def _get_coupling_electric(sol_fm, freq, n_fc, K_op_c):
    """
    ...
    if freq == 0:
        ...
        rhs_cpl_fc = K_op_c(sol_fm)

    return rhs_cpl_fc


def _get_coupling_magnetic(sol_fc, K_op_m):
    """
...
def _get_cond_fact_electric(freq, A_net_c, R_c, L_c, A_src):
    ...
    (S_mat, S_fact) = _get_cond_schur(Y_mat, A_12_mat, A_21_mat, A_22_mat, A_src)

    return Y_mat, S_mat, S_fact, A_12_mat, A_21_mat


def _get_cond_fact_magnetic(freq, A_net_m, R_m, P_m, A_src):
    """
...
def _get_cond_schur(Y_mat, A_12_mat, A_21_mat, A_22_mat, A_src):
    """
    Compute the Schur complement of a matrix.
    """

    # extract the sources
    (A_v_src, A_src_v, A_src_src) = A_src

    # computing the Schur complement (with respect to the diagonal admittance matrix)
    S_mat = A_22_mat-A_21_mat*Y_mat*A_12_mat

    # add the source
    S_mat = sps.bmat([[S_mat, A_v_src], [A_src_v, A_src_src]], dtype=np.complex_)
...
def _get_cond_solve(rhs, rhs_src, Y_mat, S_fact, A_12_mat, A_21_mat):
    """
...
def _get_system_multiply_source(sol_v, sol_src, A_src):
    """
    ...
    rhs_src_vc = A_v_src*sol_src

    # form the source equation
    rhs_1 = A_src_v*sol_v
    rhs_2 = A_src_src*sol_src
    rhs_src_src = rhs_1+rhs_2

    return rhs_src_vc, rhs_src_src


def _get_system_multiply_electric(sol_fc, sol_vc, freq, A_net_c, R_c, L_op_c):
    """
    ...
    if freq == 0:
        rhs_1 = np.zeros(len(sol_fc), dtype=np.complex_)
    else:
        rhs_1 = s*L_op_c(sol_fc)
    rhs_2 = R_c*sol_fc
    rhs_3 = -A_net_c.transpose()*sol_vc
    rhs_fc = rhs_1+rhs_2+rhs_3

    # electric KCL equations
    rhs_vc = A_net_c*sol_fc

    return rhs_fc, rhs_vc


def _get_system_multiply_magnetic(sol_fm, sol_vm, freq, A_net_m, R_m, P_op_m):
    """
    ...
    rhs_1 = R_m/s_diff*sol_fm
    rhs_2 = -A_net_m.transpose()*sol_vm
    rhs_fm = rhs_1+rhs_2

    # magnetic KCL equations
    rhs_1 = P_op_m(A_net_m*sol_fm)
    rhs_2 = s_diff*sol_vm
    rhs_vm = rhs_1+rhs_2
...
def get_source_electric(idx_v, idx_src_c, idx_src_v, G_src_c, R_src_v):
    """
    Construct the electric source matrices.
    ...
    idx_src_c_p = np.searchsorted(idx_v[idx_s], idx_src_c)
    idx_src_v_p = np.searchsorted(idx_v[idx_s], idx_src_v)
    ...
    A_v_src = sps.csc_matrix((val, (idx_row, idx_col)), shape=(n_v, n_src_c+n_src_v), dtype=np.complex_)
    ...
    A_src_v = sps.csc_matrix((val, (idx_row, idx_col)), shape=(n_src_c+n_src_v, n_v), dtype=np.complex_)
    ...
    return A_v_src, A_src_v, A_src_src


def get_source_magnetic(idx_v):
    """
    Construct the magnetic source matrices.
    ...
    # assign the matrices
    A_v_src = sps.csc_matrix((n_v, 0), dtype=np.complex_)
    A_src_v = sps.csc_matrix((0, n_v), dtype=np.complex_)
    A_src_src = sps.csc_matrix((0, 0), dtype=np.complex_)

    return A_v_src, A_src_v, A_src_src


def get_cond_operator(freq, A_net_c, A_net_m, A_src_c, A_src_m, R_c, R_m, L_c, P_m):
    """
    ...
    (n_vc, n_fc, n_src_c, n_vm, n_fm, n_src_m, n_dof) = _get_system_size(A_net_c, A_net_m, A_src_c, A_src_m)
    (scaler_fc, scaler_fm) = _get_system_scaler(freq, n_fc, n_fm)

    # get the Schur complement
    (Y_mat_c, S_mat_c, S_fact_c, A_12_mat_c, A_21_mat_c) = _get_cond_fact_electric(freq, A_net_c, R_c, L_c, A_src_c)
    (Y_mat_m, S_mat_m, S_fact_m, A_12_mat_m, A_21_mat_m) = _get_cond_fact_magnetic(freq, A_net_m, R_m, P_m, A_src_m)
    ...
    def fct(rhs):
        ...
        (rhs_c, rhs_src_c, rhs_m, rhs_src_m) = _get_split_vector(rhs, n_vc, n_fc, n_src_c, n_vm, n_fm, n_src_m)

        # solve the system (electric and magnetic)
        sol_c = _get_cond_solve(rhs_c, rhs_src_c, Y_mat_c, S_fact_c, A_12_mat_c, A_21_mat_c)
        sol_m = _get_cond_solve(rhs_m, rhs_src_m, Y_mat_m, S_fact_m, A_12_mat_m, A_21_mat_m)

        (sol_fc, sol_vc) = _get_split_field(sol_c, n_vc, n_fc)
        (sol_fm, sol_vm) = _get_split_field(sol_m, n_vm, n_fm)

        # scale and assemble the solution
        sol_fc = sol_fc/scaler_fc
        sol_fm = sol_fm/scaler_fm
...
def get_system_operator(freq, A_net_c, A_net_m, A_src_c, A_src_m, R_c, R_m, L_op_c, P_op_m, K_op_c, K_op_m):
    """
    ...
    def fct(sol):
        ...
        (sol_c, sol_src_c, sol_m, sol_src_m) = _get_split_vector(sol, n_vc, n_fc, n_src_c, n_vm, n_fm, n_src_m)
        ...
        rhs_cpl_fc = _get_coupling_electric(sol_fm, freq, n_fc, K_op_c)
        rhs_cpl_fm = _get_coupling_magnetic(sol_fc, K_op_m)

        # compute the source equation
        (rhs_src_vc, rhs_src_c) = _get_system_multiply_source(sol_vc, sol_src_c, A_src_c)
        (rhs_src_vm, rhs_src_m) = _get_system_multiply_source(sol_vm, sol_src_m, A_src_m)

        # compute the system multiplication
        (rhs_fc, rhs_vc) = _get_system_multiply_electric(sol_fc, sol_vc, freq, A_net_c, R_c, L_op_c)
        (rhs_fm, rhs_vm) = _get_system_multiply_magnetic(sol_fm, sol_vm, freq, A_net_m, R_m, P_op_m)
